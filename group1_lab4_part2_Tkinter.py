...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.geometry("550x400")  # Proper width & height for spacing
    ...
    def widgets(self):
        # Title Label
        title_label = ttk.Label(self, text="ICET Student Survey", font=("Arial", 18, "bold"))
        title_label.pack(pady=12)

        # Main Frame (Structured Look)
        frame = ttk.Frame(self, padding=15, relief="groove", borderwidth=2)
        frame.pack(fill="both", expand=True, padx=20, pady=10)

        # Labels & Entry Fields
        ttk.Label(frame, text="Full Name:").grid(row=0, column=0, sticky="w", padx=10, pady=5)
        self.name_entry = ttk.Entry(frame, width=35)
        ...
        # Button Frame (Aligned)
        button_frame = ttk.Frame(frame)
        button_frame.grid(row=6, column=0, columnspan=3, pady=15)

        ttk.Button(button_frame, text="Reset", command=self.reset_form, width=14).grid(row=0, column=0, padx=5)
        ttk.Button(button_frame, text="Submit", command=self.show_message, width=14).grid(row=0, column=1, padx=5)
        ttk.Button(button_frame, text="Exit", command=self.quit, width=14).grid(row=0, column=2, padx=5)
